...
<!-- END SIDEBAR -->
		...
		<div class="page-content">       
			...
			<!-- END PAGE HEADER-->
			<!-- BEGIN PAGE CONTENT-->
            <?php $certification = array(1=>'未认证',2=>'提交认证中',3=>'认证失败',4=>'认证成功'); ?>
			<div class="row">
				<div class="col-md-12">
						...
						<div class="portlet-body">
							...
							<table class="table table-striped table-bordered table-hover" id="sample_2">
								...
								<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="odd gradeX">
										...
										<td>
                                            <?php if(empty($$vo['neckname'])): ?>匿名用户
                                                <?php else: ?>
                                                $vo['neckname']<?php endif; ?>
                                        </td>
                                        <td>
                                            <span class="label label-sm
                <?php echo ($vo['status']=='启用'?'label-success':'label-danger'); ?>" >
                                                <?php echo ($vo["status"]); ?>
                                            </span>
                                        </td>
                                        <td>
                                            <?php echo ($certification[$vo['certification']]); ?>
                                            <?php if($admin_id == 1): ?><a href="__URL__/certification/id/<?php echo ($vo["id"]); ?>"
                                                   class="btn btn-xs yellow">
                                                    <i class="fa fa-file-o"></i> 详情</a><?php endif; ?>
                                        </td>
                                        <td width="168px">
                                            <?php if($vo["status"] == 启用): ?><a href="__URL__/update/id/<?php echo ($vo["id"]); ?>/status/0/list/dlist" 
                                                class="btn btn-xs red" style="width:50px;">
                                                    <i class="fa fa-times"></i> 禁用</a>
                                            <?php else: ?>
                                                <a href="__URL__/update/id/<?php echo ($vo["id"]); ?>/status/1/list/dlist" 
