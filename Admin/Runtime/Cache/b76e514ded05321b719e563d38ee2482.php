...
<head>
	<meta charset="utf-8" />
	<title>后台登录 | <?php echo ($sysmanage["title"]); ?></title>
	...
	<link href="__PUBLIC__/Admin/Plugins/select2/select2_metro.css" rel="stylesheet" type="text/css" />
	...
	<link href="__PUBLIC__/Admin/Css/pages/login-soft.css" rel="stylesheet" type="text/css"/>
...
<body class="login">
	<!-- BEGIN LOGO -->
	<div class="logo">
		<img src="__PUBLIC__/<?php echo ($sysmanage["logo"]); ?>" alt="" />
	</div>
	<!-- END LOGO -->
	<!-- BEGIN LOGIN -->
	<div class="content">
		<!-- BEGIN LOGIN FORM -->
		<form class="login-form" action="__URL__/check" method="post">
            <h3 class="form-title">管理员登录</h3>
            <?php if($_GET['error']== 1): ?><div class="alert alert-danger">
				<button class="close" data-close="alert"></button>
				<span>密码错误！请重新输入</span>
            </div><?php endif; ?>
            <?php if($_GET['error']== 2): ?><div class="alert alert-danger">
				<button class="close" data-close="alert"></button>
				<span>用户名不存在！</span>
            </div><?php endif; ?>
			<?php if($_GET['error']== 3): ?><div class="alert alert-danger">
					<button class="close" data-close="alert"></button>
					<span>验证码错误！</span>
				</div><?php endif; ?>
			<div class="form-group">
				...
				<div class="input-icon">
					...
					<input class="form-control placeholder-no-fix" type="text" autocomplete="off" placeholder="用户名" name="aname"/>
				</div>
			</div>
			<div class="form-group">
			...
			<div class="form-group">
				...
				<div class="input-icon">
					<i class="fa fa-lock"></i>
					<input type="text" class="form-control input-small margin" name="verify" id="verify"  style="float:left" placeholder="请输入验证码"/>
					<img src="__APP__/Public/verify/" onClick="this.src=this.src+'?'+Math.random()" style="float:right" alt="验证码"/>
				</div>
			</div>
	...
	<script src="__PUBLIC__/Admin/Plugins/jquery-validation/dist/jquery.validate.min.js" type="text/javascript"></script>
	<script src="__PUBLIC__/Admin/Plugins/backstretch/jquery.backstretch.min.js" type="text/javascript"></script>
	<script src="__PUBLIC__/Admin/Plugins/select2/select2.min.js" type="text/javascript"></script>
